...
export async function POST(request: Request) {
  ...
  const sourceAwareInstructions=`${instructions} This is interactive: the user may ask a clarifying question or say information is unknown. Never force a guess. Recognize I don't know, unknown, not sure, need to check, exclude, use benchmark, ask later, and skip for now. Route uncertainty to benchmark, estimate, unknown/verifiable, or exclusion. Every captured fact also requires sourceType (user_confirmed|user_estimate|website|industry_benchmark|system_inferred|unknown_verifiable|excluded_by_user). Confidence must be exact|estimate|range|benchmark_assumption|inferred|unknown|excluded. Guesses and ranges are user_estimate; benchmarks are never facts.`;
