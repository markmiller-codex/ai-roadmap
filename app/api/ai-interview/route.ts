...
import { detectsUncertainty } from "@/lib/discovery-issues";
...
export async function POST(request: Request) {
  ...
  if (deterministic && detectsUncertainty(answer)) { const updated=applyAnswer(assessment,deterministic,answer); const next=chooseNextQuestion(updated); const after=calculateReadiness(updated); return NextResponse.json({assistant_question:next?.title??"The uncertainty has been recorded for later verification.",rationale_for_question:"The user indicated uncertainty, so the variable was routed to an explicit benchmark, estimate, verification, or exclusion decision.",fields_targeted:["discoveryIssues"],proposed_state_updates:[],readiness_impact:{before_percent:before.percent,after_percent:after.percent,completed_sections:[]},opportunity_signals:[],next_recommended_module:next?.module??deterministic.module,updated_assessment:updated,captured_facts:[]}); }
  ...
  const sourceAwareInstructions=`${instructions} Act as an expert in the confirmed industry. Lead with website and industry assumptions and ask the user to confirm or correct them; do not ask generic tell-me-your-workflows questions. Website facts remain website/inferred until review, unchanged reviewed values become user_confirmed, and edited website values become user_corrected while retaining originalValue and sourceUrl. Compare confirmed workflows with assessment.expectedWorkflowReviews and flag common missing workflows as internal, outsourced, not material, or missed. A workflow is material if it consumes 20% of one role, 5 company hours/week, 50 occurrences/month, creates recurring delays/rework/exceptions, affects revenue/billing/collections/hiring/customer experience/compliance/visibility, relies on email/spreadsheets/documents/forms/status tracking/repeated communication, or requires repeated judgment/summarization/drafting/classification/routing/reconciliation/reporting. Do not focus on pure physical execution; capture its surrounding planning, scheduling, communication, documentation, exceptions, reporting, compliance, quality, materials, billing, and follow-up. This is interactive: the user may ask clarification or say information is unknown. Never force a guess. Route uncertainty to benchmark, estimate, unknown/verifiable, or exclusion. Every fact and workflow must carry source/confidence. Estimates and benchmarks are never confirmed facts.`;
