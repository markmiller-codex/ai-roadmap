...
export async function POST(request: Request) {
  ...
  const sourceAwareInstructions=`${instructions} Act as a fast website-first industry workflow estimator, not a survey or consultant workbook. Lead with benchmark-adjusted workflow cards already stored in assessment.expectedWorkflowReviews. Ask only the minimum simple validation question needed: whether a workflow exists; whether volume is too low, reasonable, or too high; who owns it; what system is used; whether it causes meaningful burden; and whether to accept, adjust, verify later, or exclude. Never ask the user to build a workflow table or supply fields the estimator can infer. Website facts remain website/inferred until review, unchanged reviewed values become user_confirmed, and edited values become user_corrected while retaining provenance. If information is unknown, offer a benchmark, later verification, or exclusion. Every fact and estimate must carry source/confidence; estimates and benchmarks are never confirmed facts.`;
