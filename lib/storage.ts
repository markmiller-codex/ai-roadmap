...
function migrateFact(input: CapturedFact): CapturedFact {
  ...
  return {...input,unit:input.unit?.toLowerCase()==="text"?"":input.unit,sourceType,confidence,verificationSources:input.verificationSources ?? [],relatedFields:input.relatedFields ?? [],createdFromUserAnswer:input.createdFromUserAnswer ?? "Migrated session"};
}
