...
function cleanFacts(value: unknown): CapturedFact[] | undefined {
  ...
  const confidenceValues:FactConfidence[]=["exact","estimate","range","benchmark_assumption","inferred","unknown","excluded"];
  const sourceValues:FactSourceType[]=["user_confirmed","user_corrected","user_estimate","website","industry_benchmark","system_inferred","unknown_verifiable","excluded_by_user"];
