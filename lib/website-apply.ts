...
import { generateExpectedWorkflowReviews } from "./workflow-methodology";
...
export function applyWebsiteAnalysis(assessment:Assessment,analysis:WebsiteAnalysis,inputUrl:string) {
  ...
  next.websiteDiscovery={status:"pending_review",companyName:analysis.companyName,industry:analysis.industry,businessDescription:analysis.businessDescription,services:analysis.services,customerSegments:analysis.customerSegments,locations:analysis.locations,revenueModelClues:analysis.revenueModelClues,leadershipClues:analysis.leadershipClues,hiringSignals:analysis.hiringSignals,technologyMentioned:analysis.technologyMentioned,likelyBusinessFunctions:analysis.likelyBusinessFunctions,likelyWorkflows:analysis.likelyWorkflows,complianceConstraints:analysis.complianceConstraints,sourceUrls:analysis.sourceUrls,rejectedFactLabels:[]};
  next.capturedFacts=mergeCapturedFacts(next.capturedFacts,facts); next.expectedWorkflowReviews=generateExpectedWorkflowReviews(next,analysis); next.updated_at=new Date().toISOString(); return {assessment:next,facts};
}
...
export function confirmWebsiteDiscovery(assessment:Assessment,review:Assessment["websiteDiscovery"]) { const next=structuredClone(assessment),now=new Date().toISOString(),rejected=new Set(review.rejectedFactLabels);next.websiteDiscovery={...review,status:"confirmed",confirmedAt:now};next.company_profile.company_name=review.companyName;next.company_profile.industry=review.industry;next.company_profile.operating_model=review.businessDescription;next.company_profile.customer_types=rejected.has("Customer segments")?[]:review.customerSegments;next.company_profile.revenue_sources=[...new Set([...(rejected.has("Services / products")?[]:review.services),...(rejected.has("Revenue model clues")?[]:review.revenueModelClues)])];next.company_profile.management_structure=rejected.has("Leadership / management clues")?"":review.leadershipClues.join("; ");
  const rejectedFactLabels=new Set(review.rejectedFactLabels.flatMap((label)=>({"Services / products":["Service or product"],"Customer segments":["Customer segment"],"Locations":["Location"],"Revenue model clues":["Revenue model clue"],"Leadership / management clues":["Leadership clue"],"Hiring / career clues":["Hiring signal"],"Technology clues":["Technology mentioned"],"Likely business functions":["Likely business function"],"Likely workflows":["Likely workflow"],"Compliance or industry constraints":["Compliance constraint"]}[label]??[label])));next.capturedFacts=next.capturedFacts.filter((fact)=>fact.sourceType!=="website"||!rejectedFactLabels.has(fact.label)).map((fact)=>fact.sourceType==="website"?{...fact,sourceType:"user_confirmed" as const,confidence:"exact" as const,needsConfirmation:false,confirmedByUser:true}:fact);next.expectedWorkflowReviews=generateExpectedWorkflowReviews(next);next.updated_at=now;return next; }
